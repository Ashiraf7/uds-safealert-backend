...
router.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      // alertRadius may come as "3km (recommended)" — extract the number and convert to metres
      const radiusRaw = String(alertRadius || '500');
      const radiusNum = parseFloat(radiusRaw.replace(/[^0-9.]/g, '')) || 500;
      const radiusM = radiusRaw.toLowerCase().includes('km') ? Math.round(radiusNum * 1000) : Math.round(radiusNum);
